...
export class SpeechSynthesisRecorder {
	...
	//TODO: add default options so that nothing is left undefined (low priority)
	constructor({ text = '', utteranceOptions = { lang: 'en-US', rate: 1, pitch: 0.75, volume: 1 } as any, recorderOptions = {}, dataType = '' }) {
		...
		this.chunks = [];
		if (utteranceOptions) {
			// console.log('Utterance options:', utteranceOptions);

			if (utteranceOptions.voice) {
				this.speechSynthesis.onvoiceschanged = (e) => {
					const voice = this.speechSynthesis.getVoices().find(({ name: _name }) => _name === utteranceOptions.voice);
					this.utterance.voice = voice ?? null;
					console.log('Utterance:', voice, this.utterance);
				};
			...
			// console.log('Rate and volume:', rate, volume);
						...
						new Promise((resolve) => {
							...
							this.mediaRecorder.ondataavailable = (event) => {
								if (event.data.size > 0) {
									this.chunks.push(event.data as never);
								}
							};
	...
	arrayBuffer(blob?: any) {
		if (!this.chunks.length) throw new Error('no data to return');
		return new Promise<{ tts: any; data: string | ArrayBuffer | null }>((resolve) => {
	...
	audioBuffer() {
		...
		return this.arrayBuffer()
			.then(({ tts, data }) => this.audioContext.decodeAudioData(data as ArrayBuffer))
			.then((buffer) => ({
				tts: this,
				data: buffer
			}));
...
if (typeof window !== 'undefined') (window as any).SpeechSynthesisRecorder = SpeechSynthesisRecorder;
